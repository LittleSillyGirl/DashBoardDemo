...
public class DashBoardView extends View implements  Runnable{
    ...
    private final String TAG = DashBoardView.class.getSimpleName();
    private Context mContext ;
    private OnUpdateProgress mOnUpdateProgress;
...
//        canvas.drawArc(mRectFOuter, defaultAngle - 3, sweepAngle+ 6, false, mPaint);
    ...
    private void setPercentValueAnimate(Canvas canvas, float xStart, float yStart, float xEndLong, float yEndLong,
        ...
        if (preAngle <= percentAngle1){
            ...
            endAngle = preAngle ;
        }else if (preAngle <= percentAngle1 + percentAngle2 ){
            ...
            mPaint.setColor(res.getColor(percentAgeColors[0]));
            canvas.drawArc(mRectFInner, getDefaultAngle(), percentAngle1, false, mPaint);

            mPaint.setColor(res.getColor(percentAgeColors[1]));
            startAngle = getDefaultAngle() +  percentAngle1;
            endAngle = preAngle - percentAngle1;
        }else if (preAngle <= percentAngle1 + percentAngle2 + percentAngle3){
            ...
            canvas.drawArc(mRectFInner, getDefaultAngle(), percentAngle1, false, mPaint);
            mPaint.setColor(res.getColor(percentAgeColors[1]));
            canvas.drawArc(mRectFInner, getDefaultAngle()+ percentAngle1, percentAngle2, false, mPaint);

            mPaint.setColor(res.getColor(percentAgeColors[2]));
            startAngle = getDefaultAngle() +  percentAngle1 + percentAngle2 ;
            endAngle = preAngle - percentAngle1 - percentAngle2;
        }else if (preAngle <= sweepAngle){
            ...
            canvas.drawArc(mRectFInner, getDefaultAngle()+ percentAngle1, percentAngle2, false, mPaint);
            mPaint.setColor(res.getColor(percentAgeColors[2]));
            canvas.drawArc(mRectFInner, getDefaultAngle()+ percentAngle1 + percentAngle2 , percentAngle3, false, mPaint);

            mPaint.setColor(res.getColor(percentAgeColors[3]));
            startAngle = getDefaultAngle() +  percentAngle1 + percentAngle2 + percentAngle3;
...
//        Log.i(TAG, "=1444===刻度线个数=preScaleNum4=="+ preScaleNum4);
    ...
    public void run() {
        if (isAnimate){
            ...
            if (percentAges.length>0){
                //TODO 如果有等份我们就开始动画
                preAngle++;
                mOnUpdateProgress.updateProgress(preAngle);
                try {
    ...
    public OnUpdateProgress getmOnUpdateProgress() {
        return mOnUpdateProgress;
    }

    public void setmOnUpdateProgress(OnUpdateProgress mOnUpdateProgress) {
        this.mOnUpdateProgress = mOnUpdateProgress;
    }
